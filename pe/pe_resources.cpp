...
PEResources::ResourceItem PEResources::find(const String &name) const { return this->find(name, m_resourcedirectory); }
...
PEResources::ResourceItem PEResources::find(const String &name, const PEResources::ResourceItem &parentres) const
{
...
PEResources::ResourceItem PEResources::find(const String &name, ImageResourceDirectory *resourcedir) const
{
    ...
    for(u32 i = 0; i < c; i++)
    {
        String n = this->entryName(&entries[i]);
...
String PEResources::entryName(ImageResourceDirectoryEntry *entry) const
{
    if(entry->NameIsString)
    {
        ImageResourceDirStringU* name = RESOURCE_PTR(ImageResourceDirStringU, m_resourcedirectory, entry->NameOffset);
        return String::wide(name->NameString, name->Length);
    }

    return this->resourceid(entry->Id);
}

String PEResources::resourceid(u16 id) const
{
