...
DexDataMap* dexDataMapAlloc(dex_u4 maxCount) {
    ...
    const dex_u4 sizeOfItems = (dex_u4) (sizeof(dex_u4) + sizeof(dex_u2));
    ...
    map->count = 0;
    map->max = maxCount;
    map->offsets = (dex_u4*) (map + 1);
    map->types = (dex_u2*) (map->offsets + maxCount);
...
int dexDataMapGet(DexDataMap* map, dex_u4 offset) {
    ...
    int min = 0;
    int max = map->count - 1;
    dex_u4* offsets = map->offsets;

    while (max >= min) {
        int guessIdx = (min + max) >> 1;
        dex_u4 guess = offsets[guessIdx];
