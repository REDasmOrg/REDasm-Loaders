...
static const char* readStringIdx(const DexFile* pDexFile,
        const dex_u1** pStream) {
    dex_u4 stringIdx = readUnsignedLeb128(pStream);
...
static const char* readTypeIdx(const DexFile* pDexFile,
        const dex_u1** pStream) {
    dex_u4 typeIdx = readUnsignedLeb128(pStream);
...
static void emitLocalCbIfLive(void *cnxt, int reg, dex_u4 endAddress,
            ...
            LocalInfo* localInReg)
{
    ...
    dex_u4 line = readUnsignedLeb128(&stream);
    dex_u4 parametersSize = readUnsignedLeb128(&stream);
    dex_u2 argReg = pCode->registersSize - pCode->insSize;
    dex_u4 address = 0;
