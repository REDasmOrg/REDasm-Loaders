...
DEX_INLINE const DexStringId* dexGetStringId(const DexFile* pDexFile, dex_u4 idx) {
...
DEX_INLINE const DexMethodId* dexGetMethodId(const DexFile* pDexFile, dex_u4 idx) {
...
DEX_INLINE const DexFieldId* dexGetFieldId(const DexFile* pDexFile, dex_u4 idx) {
...
DEX_INLINE const DexProtoId* dexGetProtoId(const DexFile* pDexFile, dex_u4 idx) {
...
DEX_INLINE const DexClassDef* dexGetClassDef(const DexFile* pDexFile, dex_u4 idx) {
...
DEX_INLINE const DexAnnotationSetItem* dexGetAnnotationSetItem(
    const DexFile* pDexFile, dex_u4 offset)
{
    if (offset == 0) {
        return NULL;
    }
...
DEX_INLINE const DexAnnotationSetRefItem* dexGetParameterAnnotationSetRef(
    const DexAnnotationSetRefList* pList, dex_u4 idx)
{
    assert(idx < pList->size);
    return &pList->list[idx];
}
...
DEX_INLINE dex_u4 dexGetAnnotationOff(
...
DEX_INLINE const DexAnnotationItem* dexGetAnnotationItem(
    const DexFile* pDexFile, const DexAnnotationSetItem* pAnnoSet, dex_u4 idx)
{
