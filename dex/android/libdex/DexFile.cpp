 ...
 */
static dex_u4 classDescriptorHash(const char* str)
{
    dex_u4 hash = 1;
    ...
    int stringOff, int classDefOff, int* pNumProbes)
{
    ...
    dex_u4 hash = classDescriptorHash(classDescriptor);
...
DexClassLookup* dexCreateClassLookup(DexFile* pDexFile)
{
    ...
    for (i = 0; i < (int)pDexFile->pHeader->classDefsSize; i++) {
        ...
        classLookupAdd(pDexFile, pLookup,
            (dex_u1*)pString - pDexFile->baseAddr,
            (dex_u1*)pClassDef - pDexFile->baseAddr, &numProbes);
...
DexFile* dexFileParse(const dex_u1* data, size_t length, int flags)
{
    ...
    if (flags & kDexParseVerifyChecksum) {
        dex_u4 adler = dexComputeChecksum(pHeader);
...
size_t dexGetDexCodeSize(const DexCode* pCode)
{
    ...
    const dex_u1* handlerData = dexGetCatchHandlerData(pCode);
    ...
    return (handlerData - (dex_u1*) pCode) + offset;
}
