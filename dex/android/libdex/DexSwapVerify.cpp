...
struct CheckState {
    ...
    const dex_u1*         fileStart;
    const dex_u1*         fileEnd;             // points to fileStart + fileLen
    dex_u4                fileLen;
 ...
 */
static inline dex_u4 fileOffset(const CheckState* state, const void* ptr) {
    return ((const dex_u1*) ptr) - state->fileStart;
}

/*
 * Return a pointer for the given file offset.
 */
static inline void* filePointer(const CheckState* state, dex_u4 offset) {
...
#define CHECK_OFFSET_RANGE(_start, _end) {                                  \
        const dex_u1* _startPtr = (const dex_u1*) filePointer(state, (_start));     \
        const dex_u1* _endPtr = (const dex_u1*) filePointer(state, (_end));         \
...
#define CHECK_LIST_SIZE(_ptr, _count, _elemSize) {                          \
        const dex_u1* _start = (const dex_u1*) (_ptr);                              \
        const dex_u1* _end = _start + ((_count) * (_elemSize));                 \
        dex_u4 _dummy;                                                          \
...
#define CHECK_INDEX(_field, _limit) {                                       \
        if ((_field) >= (_limit)) {                                         \
            ALOGW("Bad index: %s(%u) > %s(%u)",                             \
                #_field, (dex_u4)(_field), #_limit, (dex_u4)(_limit));              \
            return 0;                                                       \
        }                                                                   \
    }
...
static bool verifyFieldDefiner(const CheckState* state, dex_u4 definingClass,
        dex_u4 fieldIdx) {
...
static bool verifyMethodDefiner(const CheckState* state, dex_u4 definingClass,
        dex_u4 methodIdx) {
 ...
 */
static bool setDefinedClassBit(const CheckState* state, dex_u4 typeIdx) {
    dex_u4 arrayIdx = typeIdx >> 5;
    dex_u4 bit = 1 << (typeIdx & 0x1f);
    dex_u4* element = &state->pDefinedClassBits[arrayIdx];
...
static bool swapDexHeader(const CheckState* state, DexHeader* pHeader)
{
    ...
    dex_u4 linkOff = pHeader->linkOff;
    dex_u4 linkEnd = linkOff + pHeader->linkSize;
    dex_u4 dataOff = pHeader->dataOff;
    dex_u4 dataEnd = dataOff + pHeader->dataSize;
...
static bool checkHeaderSection(const CheckState* state, dex_u4 sectionOffset,
 ...
 */
static dex_u4 mapTypeToBitMask(int mapType) {
    switch (mapType) {
...
/* defined below */
static dex_u4 findFirstClassDataDefiner(const CheckState* state,
        DexClassData* classData);
static dex_u4 findFirstAnnotationsDirectoryDefiner(const CheckState* state,
...
static bool verifyClassDataIsForDef(const CheckState* state, dex_u4 offset,
        dex_u4 definerIdx) {
    if (offset == 0) {
        return true;
    }

    const dex_u1* data = (const dex_u1*) filePointer(state, offset);
     ...
     */
    dex_u4 dataDefiner = findFirstClassDataDefiner(state, classData);
...
static bool verifyAnnotationsDirectoryIsForDef(const CheckState* state,
        dex_u4 offset, dex_u4 definerIdx) {
    if (offset == 0) {
        return true;
    }
    ...
    dex_u4 annoDefiner = findFirstAnnotationsDirectoryDefiner(state, dir);
...
static bool verifyFields(const CheckState* state, dex_u4 size,
    ...
    for (i = 0; i < size; i++) {
        ...
        dex_u4 accessFlags = field->accessFlags;
...
static bool verifyMethods(const CheckState* state, dex_u4 size,
    ...
    for (i = 0; i < size; i++) {
        ...
        dex_u4 accessFlags = method->accessFlags;
        bool isDirect =
...
static dex_u4 findFirstClassDataDefiner(const CheckState* state,
    ...
    if (classData->header.staticFieldsSize != 0) {
        dex_u4 fieldIdx = classData->staticFields[0].fieldIdx;
        const DexFieldId* field = dexGetFieldId(state->pDexFile, fieldIdx);
        return field->classIdx;
    }

    if (classData->header.instanceFieldsSize != 0) {
        dex_u4 fieldIdx = classData->instanceFields[0].fieldIdx;
        const DexFieldId* field = dexGetFieldId(state->pDexFile, fieldIdx);
        return field->classIdx;
    }

    if (classData->header.directMethodsSize != 0) {
        dex_u4 methodIdx = classData->directMethods[0].methodIdx;
        const DexMethodId* meth = dexGetMethodId(state->pDexFile, methodIdx);
        return meth->classIdx;
    }

    if (classData->header.virtualMethodsSize != 0) {
        dex_u4 methodIdx = classData->virtualMethods[0].methodIdx;
        const DexMethodId* meth = dexGetMethodId(state->pDexFile, methodIdx);
        return meth->classIdx;
    }
...
static const dex_u1* verifyEncodedValue(const CheckState* state, const dex_u1* data,
        bool crossVerify);
static const dex_u1* verifyEncodedAnnotation(const CheckState* state,
        const dex_u1* data, bool crossVerify);

/* Helper for verifyEncodedValue(), which reads a 1- to 4- byte unsigned
 * little endian value. */
static dex_u4 readUnsignedLittleEndian(const CheckState* state, const dex_u1** pData,
        dex_u4 size) {
    const dex_u1* data = *pData;
    dex_u4 result = 0;
    dex_u4 i;
...
static const dex_u1* verifyEncodedArray(const CheckState* state,
        const dex_u1* data, bool crossVerify) {
    bool okay = true;
    dex_u4 size = readAndVerifyUnsignedLeb128(&data, state->fileEnd, &okay);
...
static dex_u4 numberOfMethodHandles(const CheckState* state) {
    if (state->pMethodHandleItems != nullptr) {
...
static const dex_u1* verifyEncodedValue(const CheckState* state,
        const dex_u1* data, bool crossVerify) {
    CHECK_PTR_RANGE(data, data + 1);

    dex_u1 headerByte = *(data++);
    dex_u4 valueType = headerByte & kDexAnnotationValueTypeMask;
    dex_u4 valueArg = headerByte >> kDexAnnotationValueArgShift;
    ...
    switch (valueType) {
        ...
        case kDexAnnotationMethodType: {
            ...
            dex_u4 idx = readUnsignedLittleEndian(state, &data, valueArg + 1);
...
static const dex_u1* verifyEncodedAnnotation(const CheckState* state,
    ...
    dex_u4 size = readAndVerifyUnsignedLeb128(&data, fileEnd, &okay);
    dex_u4 lastIdx = 0;
    bool first = true;
...
static void* intraVerifyEncodedArrayItem(const CheckState* state, void* ptr) {
    return (void*) verifyEncodedArray(state, (const dex_u1*) ptr, false);
}
...
{
    const dex_u1* magic = pHeader->magic;
    const dex_u1* version = &magic[4];
